...
const int no_details_flag = 0;
const int no_discards = 0;
...
static void build_script(struct file_data const[], add_change_callback callback, void* userData);
...
discard_confusing_lines (struct file_data filevec[])
{
  ...
  discarded[0] = (char *)malloc (sizeof (char)
...
static void
build_script (struct file_data const filevec[], add_change_callback callback, void* userData)
{
  ...
  while (i0 >= 0 || i1 >= 0)
      ...
      if (changed0[i0 - 1] || changed1[i1 - 1])
	{
	  ...
	  /* Record this change.  */
	  callback (i0, i1, line0 - i0, line1 - i1, userData);
	}
